			...
			for (int orientation = 0; orientation < 6; orientation++) {
				for (int z = 0; z < length; z++)
					for (int x = 0; x < width; x++) {
						...
						position = CubeVertexToSphere(position);*/
						//position *= Radius + cubeHeights[orientation][x, z];
						if (orientation <= 4) position *= Radius + cubeHeights[orientation][x, z];
						else position *= Radius;// ok
